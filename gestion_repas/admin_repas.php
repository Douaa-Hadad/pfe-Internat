...
if (!isset($_SESSION['user_type']) || $_SESSION['user_role'] !== 'Gestionnaire_Tickets') {
...
if (isset($_POST['submit'])) {
    ...
    // Vérifier si la date sélectionnée est différente de la date actuelle
    $current_date = date('Y-m-d');
    if ($date_repas !== $current_date) {
        $error_message = "Vous ne pouvez générer des tickets que pour la date d'aujourd'hui.";
    } else {
        ...
        if (mysqli_num_rows($result_check_existing) > 0) {
            $error_message = "Les tickets pour ce repas ont déjà été générés pour cette date.";
        } else {
            ...
            if (mysqli_num_rows($result) > 0) {
                ...
                $success_message = "Les tickets ont été générés avec succès pour tous les étudiants !";
            } else {
                $error_message = "Aucun étudiant trouvé dans la base de données.";
            }
        }
    }
}
